class RecordsController < ApplicationController
  ...
  def filter
    ...
    @records = if start_date > end_date
                 Record.accessible_by(current_ability).where('date >= ?', end_date).where('date <= ?', start_date)
               else
                 Record.accessible_by(current_ability).where('date <= ?', end_date).where('date >= ?', start_date)
               end
    render json: @records
  ...
  def create
    ...
    @record = Record.new(time:, distance:, date:, member: @member, speed:)
    if @record.save
  ...
  def update
    ...
    distance = params[:record][:distance].to_f
    speed = distance / (params[:record][:time][:hours] + (params[:record][:time][:minutes] / 60.to_f))
    if @record.update(time:, distance:, date:, speed:)
  ...
  def record_params
    params.require(:record).permit(:date, :distance, { time: %i[hours minutes] })
  end
end
